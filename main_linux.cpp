// main.cpp — мультиплатформенная версия (Windows / Linux)
// Использует ascii_render.hpp (ваш файл) без изменений.
// Поддерживает динамическую загрузку libvlc (LoadLibrary / dlopen).
// Windows-часть максимально совпадает с вашим исходником.
// Linux-часть использует ncurses для ввода и dlopen для libvlc.
...
// --- platform includes / helpers ---
    ...
    void set_console_size(int width, int height) {
        // Try to set the pty window size (may or may not change emulator window)
        struct winsize ws;
        ws.ws_col = width;
        ws.ws_row = height;
        ws.ws_xpixel = 0;
...
int main(int argc, char* argv[]) {
    ...
    int width = 200; // lower -> better perf
